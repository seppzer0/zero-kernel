...
class GenericContainerEngine(ModelConfig, IGenericContainerEngine):
    ...
    :param typing.Optional[bool]=False ksu: Flag to add KernelSU support into the kernel.
    ...
    def check_cache(self) -> bool:
        img_cache_cmd = f'{self.benv.value} image list --format {"{{.Repository}}"}'
    ...
    def builder_cmd(self) -> str:
        ...
        if self.command == EnumCommand.BUNDLE:
            if self.package_type in (EnumPackageType.SLIM, EnumPackageType.FULL):
        ...
        match self.command:
            ...
            case EnumCommand.BUNDLE:
                match self.package_type:
                    case EnumPackageType.SLIM | EnumPackageType.FULL:
    ...
    def build_image(self) -> str | None | CompletedProcess:
        print("\n")
        log.warning(f"Building the {self.benv.value.capitalize()} image..")
    ...
    def __exit__(self, exc_type, exc_value, traceback) -> None:
        ...
        if self.clean_image:
            ccmd.launch(f"{self.benv.value} rmi {self._name_image}")
