...
class RomApiClient(ModelConfig, IRomApiClient):
    ...
    :param zkb.enums.EnumKernelBase rom_name: ROM project's name.
    :param bool rom_only: Flag indicating ROM-only asset collection.
    """

    endpoint: str
    json_key: str
    rom_name: EnumKernelBase
    codename: str
    rom_only: bool
    ...
    def run(self) -> str:
        ...
        try:
            ...
            log.error(f"Could not connect to {self.rom_name.value} API, HTTP status code: {data.status_code}")
